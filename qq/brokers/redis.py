...
class RedisBroker(Broker):
    ...
    async def enqueue(self, msg: Message) -> None:
        ...
        await self.r.xadd(self._stream(msg.queue), {"m": msg.marshal()})
    ...
    async def schedule(self, msg: Message, not_before: datetime) -> None:
        ...
        await self.r.zadd(self._zset(msg.queue), {msg.marshal(): score})
    ...
    ) -> AsyncIterator[Delivery[tuple[str, bytes]]]:
        for q in queues:
                ...
                while True:
                    for q in queues:
                        for sid, data in await self._claim_stale(q):
                            yield Delivery(
                                message=Message.unmarshal(data[b"m"]),
                                receipt=(q, sid),
                                queue=q,
                            )
    ...
    async def ack(self, delivery: Delivery[tuple[str, bytes]]) -> None:
    ...
    async def nack(
        self,
        delivery: Delivery[tuple[str, bytes]],
        requeue: bool = True,
        ...
        if delay and delay > 0:
            ...
            await self.r.zadd(self._zset(q), {msg.marshal(): when})
        else:
            await self.r.xadd(self._stream(q), {"m": msg.marshal()})
        await self.r.xack(self._stream(q), self.group, sid)
        await self.r.xdel(self._stream(q), sid)
