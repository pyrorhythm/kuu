...
class Orchestrator:
	...
	async def start(self) -> None:
		"""
		Run the orchestrator until SIGINT/SIGTERM or programmatic shutdown.

		Spawns the worker pool, dashboard, watcher and scheduler concurrently
		under a single task group; tears them all down when `_stop_event` is
		set. Safe to call from a host event loop.
		"""
