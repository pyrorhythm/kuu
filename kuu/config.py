...
class Kuunfig(BaseModel):
	...
	def load(cls, config: str | pathlib.Path | None = None) -> Self:
		...
		if `config` is None, autodiscover in cwd: prefer `kuunfig.toml`, then
		`pyproject.toml` (read from `[tool.kuu]`)
		"""
	...
	def with_overrides(self, overrides: list[str]) -> Self:
		...
		values are parsed as JSON when possible (so `true`, `42`, `"x"`, `[1,2]`
		all work); otherwise they are taken as a raw string.
		"""
		if not overrides:
			return self
